...
class Register(APIView):
    ...
    def post(self, request, format=None):
        url = "http://192.168.1.100:8000/register/"
        ...
        if category == 'Repairer':
            ...
            try:
                ...
                shoppid = shopp
            repairer = Repairer.objects.create_user(email=email, password=password, fname=fname, lname=lname,
                                                    uname=uname, phone=phone, secretque=secretque, secretans=secretans,
                                                    cnic=cnic, lon=lon, lat=lat, shopid=shoppid)
            ...
            customer = Customer.objects.create_user(email=email, password=password, fname=fname, lname=lname,
                                                    uname=uname, phone=phone, secretque=secretque, secretans=secretans,
                                                    cnic=cnic, lon=lon, lat=lat)
...
class EditRepairerProfile(APIView):
    parser_classes = (JSONParser,)
    def post(self, request, format=None):
        url = "http://192.168.0.7:8000/editRepairerProfile/"
        body_unicode = request.body.decode('utf-8')
        userdata = json.loads(body_unicode)
        ...
        try:
            ...
            if user:
                ...
                # try:
                #     shopId = user.shopid
                #     shopp = Shop(id=shopId, name=shop)
                # except Shop.DoesNotExist:
                #
                #     shopp.save()
                #     shoppid = shopp
                ...
                # shopId = user.shopid
                # print(shopId)
                # shopp = Shop.objects.get(id=shopId)
                # print(shopp)
                # shopp.name = shop
                # print(shop)
                # shopp.save()
...
class EditExpertise(APIView):
    ...
    def post(self, request, format=None):
        url="http://192.168.0.7:8000/editExpertise/"
        body_unicode = request.body.decode('utf-8')
        userdata = json.loads(body_unicode)
        ...
        # print(category)
        uname = userdata['uname']
        repairer = Repairer.objects.get(username=uname)
        print(repairer)
        #expList = expertise.split(',')

        expp = Expertise.objects.filter(username=repairer)
        print(expp)
        expp.delete()
        # l = len(expp.workCategory)
        # for exp in l[:-1]:
        #     expertisse = Expertise(username=repairer, workCategory=exp)
        #     #print (exp)
        #     # expertisse.username = uname
        #     expertisse.delete()
       ...
       #  lat=userdata['latitude']
        ...
        # repairer.save()
        # exp = Expertise.objects.get(username=uname)
        # if (exp):
        #     for i in exp:
        #         expList.append(i.workCategory)
        #     userdata['expertise'] = expList
        #     expertise.save()

        # expertisse=Expertise(username=repairer,workCategory=expertise)
        # expertisse.save()
        # expertise.username = expertisse
        # repairer.expertise = expertisse
        # expertisse.username = uname
        # elif category=='Client':
